package com.gson8.mycalendarview.calutils;

import java.text.SimpleDateFormat;
import java.util.*;

/**
 * Making by Syusuke on 2016/9/11
 * Email: Zyj7810@126.com
 * Name: MainLunar
 * Description: null
 */

/**
 * getLunarDate()       获取详细农历日期
 * getLunarMonth()      农历月份
 * getLunarDay()        农历月日 如: 七月初一
 * getLunarTime()       农历时间 如: 己酉时
 * getLunarHoliday()    农历的节日 如:七夕
 * getHoliday()         公历的节日 如:元旦
 * getCalendar()        农历信息以Calendar形式返回
 * getLunarDayOfMonth() 农历日期 : 如: 初一
 * getSoralTerm()       24节气
 * <p/>
 * nextDay()            当前日期的下一天
 * preDay()             当前日期的前一天
...
public class Lauar {
    ...
    private void convert() {
        ...
        // 偏移量（天）
        int offset = (int) ((calendar.getTimeInMillis() - baseDate.getTime()) /
                86400000); // 天数(86400000=24*60*60*1000)
        // 基准时间在天干地支纪年法中的位置
    ...
    public String getMyLauar() {

        if(getLunarDay().contains("正月初一")) {
            int y = getCalendar().get(Calendar.YEAR);
            return ("" + y).substring(2) + "/" + getLunarMonth();
        }

        //农历节日,春节,七夕
        if(getLunarHoliday() != null)
            return getLunarHoliday();

        //24 节气
        if(getSoralTerm() != null)
            return getSoralTerm();

        if(getHoliday() != null)
            return getHoliday();

        return getLunarDayOfMonth();
    }
...
/*    public static void main(String[] args) {
        ...
        Lauar lauar = new Lauar(2016, 8, 9);

        System.out.println(lauar.getLunarDate());

        System.out.println(lauar.getLunarMonth() + "  ");
        System.out.println(lauar.getLunarDay() + "  ");
        System.out.println(lauar.getLunarTime() + "  ");
        System.out.println(lauar.getLunarHoliday() + "  节日");
        System.out.println(lauar.getHoliday());

        Calendar calendar = lauar.getCalendar();
        System.out
                .println(calendar.get(Calendar.YEAR) + "  " + calendar.get(Calendar.MONTH) + "  " +
                        calendar.get(Calendar.DAY_OF_MONTH));

        System.out.println(lauar.getLunarDayOfMonth() + "  - ");
